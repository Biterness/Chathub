...
namespace Chathub.API.Domain.UnitOfWorks.Implementation
{
    public class ChatRoomUnitOfWork : IChatRoomUnitOfWork
    {
        ...
        {
            var roomList = await _chatMemberRepository.GetAsync(member => member.UserId == userId);
            return await _chatRoomRepository.GetWithAsync(room => roomList.Exists(r => r.ChatRoomId == room.Id), room => room.ChatContents.OrderByDescending(message => message.CreatedAt).Take(100), room => room.ChatMembers);
        }
